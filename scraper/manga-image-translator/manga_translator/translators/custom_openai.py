...
class CustomOpenAiTranslator(ConfigGPT, CommonTranslator):
    ...
    def __init__(self, model=None, api_base=None, api_key=None, check_openai_key=False):
        ...
        const_api_key = api_key or CUSTOM_OPENAI_API_KEY or "ollama"
        self.client = openai.AsyncOpenAI(api_key=const_api_key, default_headers={"x-goog-api-key": const_api_key})
    ...
    async def _request_translation(self, to_lang: str, prompt: str) -> str:
        ...
        sys_prompt = self.chat_system_template.format(to_lang=to_lang)
        
        contents = []
        # Add chat samples if available
        lang_chat_samples = self.get_chat_sample(to_lang)
        if lang_chat_samples:
            contents.append({"role": "user", "parts": [{"text": lang_chat_samples[0]}]})
            contents.append({"role": "model", "parts": [{"text": lang_chat_samples[1]}]})

        contents.append({"role": "user", "parts": [{"text": prompt}]})

        model_name = self.model or "gemini-1.5-pro-latest"
        # If it has openai/ prefix from ENV, strip it
        if "gemini" in model_name and "/" in model_name:
            model_name = model_name.split("/")[-1]
            
        url = f"https://generativelanguage.googleapis.com/v1beta/models/{model_name}:generateContent?key={CUSTOM_OPENAI_API_KEY}"
