 ...
 * ║   Hedef: ragnarscans.com/avatar-frame/                      ║
 * ║   Görev: Tüm avatar çerçevelerini Cloudflare R2'ye yükle    ║
...
const R2_PREFIX = 'avatar-frames/';

// ─── HEDEF SAYFALARI ────────────────────────────────────────────
const BASE_URL = 'https://ragnarscans.com/avatar-frame/';
const PAGES = [
  BASE_URL,
  `${BASE_URL}page/2/`,
  `${BASE_URL}page/3/`,
...
async function ensureDownloadPage(browser) {
  if (!_dlPage || _dlPage.isClosed()) {
    ...
    await _dlPage.goto('https://ragnarscans.com/avatar-frame/', { 
...
async function extractFramesFromPage(browser, pageUrl) {
  ...
  // frame-card içindeki tüm çerçeve imglerini çek
  const frames = await page.evaluate(() => {
    ...
    cards.forEach(card => {
      ...
      // Görseli al (frame-card içindeki ana img — logo/icon değil)
      const img = card.querySelector('.uk-text-center img');
      if (!img) return;
      
      const src = img.getAttribute('src') || img.getAttribute('data-src') || '';
      if (!src) return;
      
      // Sadece wp-content/uploads altındaki görselleri al (site logosu vs. değil)
...
║   Hedef: ragnarscans.com/avatar-frame/                      ║
║   Görev: Tüm çerçeveleri R2'ye yükle                        ║
╚══════════════════════════════════════════════════════════════╝
`);
  ...
  try {
    ...
    console.log(`\n${'━'.repeat(60)}`);
    console.log(`[TOPLAM] >> ${allFrames.length} benzersiz avatar çerçevesi bulundu.`);
    ...
    try {
      ...
      const R2_PUBLIC_URL = 'https://pub-56389f4fc14f4af4b80a25136a28126e.r2.dev/avatar-frames';
      let addedToSite = 0;
      ...
      for (const frame of allFrames) {
        ...
        if (!alreadyExists) {
          effectsData.push({
            id: `avatar_${filename.split('.')[0].replace(/[^a-zA-Z0-9]/g, '_')}_${Date.now()}`,
            url: fullUrl,
            label: frame.name,
      ...
      if (addedToSite > 0) {
        ...
        console.log(`   [JSON ✓] >> ${addedToSite} adet yeni avatar efekti web sitesine (effects.json) eklendi!`);
      } else {
