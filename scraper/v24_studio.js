...
async function main() {
  ...
  console.log(`\x1b[32m[SYSTEM]\x1b[0m >> Aktif Python Motoru: ${activePython}\n`);
  ...
  console.log('\x1b[35m[2] FINAL AI EDIT:\x1b[0m AI ile işle, \\EDIT\\ klasörüne pırlanta gibi istifle!');
  console.log();

  const modeInput = await ask('\x1b[33m[SEÇİM]\x1b[0m >> Operasyon modunu seçin (1/2): ');
  const isAiEdit = modeInput === '2';
  const modeLabel = isAiEdit ? 'FINAL_AI_EDIT' : 'REBRAND_SPEED';
  ...
  for (const target of targets) {
    ...
    try {
      ...
      // Browser
      if (!browser) {
        console.log('\x1b[33m[BROWSER]\x1b[0m >> Final-Reaper Siber Motor Başlatılıyor...');
      ...
      // Pipeline
      console.log(`\n\x1b[31m[FINAL-REAPER]\x1b[0m >> ${seriesData.chapters.length} bölüm namluya sürüldü...\n`);
      ...
      for (const chapter of seriesData.chapters) {
        ...
        // Step 3: Distribution (Key Pool Rotation)
      ...
      logger.error(`[Final-Reaper] Kritik Hata: ${err.message}`);
