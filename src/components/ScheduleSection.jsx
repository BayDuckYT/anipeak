    ...
    <section id="yayin-takvimi" className="py-20 px-4 max-w-7xl mx-auto overflow-hidden">
          ...
          return (
            <motion.button
              ...
              className={`relative p-6 rounded-[2rem] border transition-all duration-500 text-center group overflow-hidden ${
                ...
                : 'bg-white/40 border-purple-900/5 hover:border-purple-900/10'
              }`}
            >
              {isActive && (
                <div className="absolute top-2 right-2 w-2 h-2 bg-emerald-400 rounded-full animate-pulse" />
              )}
              <span className={`block text-sm font-black uppercase tracking-widest mb-1 ${isActive ? 'text-slate-900' : 'text-slate-600 group-hover:text-slate-900 transition-colors'}`}>
                {day.name}
              </span>
              <span className={`block text-[10px] font-bold opacity-60 mb-3 ${isActive ? 'text-slate-900' : 'text-zinc-500'}`}>
              ...
              <div className={`w-8 h-8 mx-auto rounded-full flex items-center justify-center text-xs font-black ${
                isActive ? 'bg-slate-900/20 text-slate-900' : 'bg-slate-900/5 text-slate-600'
              }`}>
                {dayCount}
              </div>
            </motion.button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 space-y-6">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight">{DAYS[selectedDay].name} Programı</h3>
            </div>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              {dailyPrograms.length > 0 ? dailyPrograms.map((item) => (
                <div key={item.id} className="group relative glass bg-white/40 border border-purple-900/5 rounded-3xl p-5 hover:border-indigo-500/30 transition-all flex items-center gap-6">
                  <div className="flex flex-col items-center justify-center min-w-[80px] text-center border-r border-purple-900/5 pr-6">
                    <span className="text-2xl font-black text-slate-900">{item.release_time.slice(0, 5)}</span>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <h4 className="text-lg font-black text-slate-900 uppercase tracking-tight truncate group-hover:text-indigo-400 transition-colors">{item.series_name}</h4>
                    </div>
                  ...
                  <button className="p-3 rounded-2xl bg-slate-900/5 text-zinc-500 opacity-0 group-hover:opacity-100 group-hover:text-slate-900 transition-all">
                    <ChevronRight size={20} />
                  </button>
                </div>
              )) : (
                <div className="py-20 text-center glass border border-dashed border-purple-900/10 rounded-[2.5rem]">
        ...
        <div className="lg:col-span-4">
          <div className="glass bg-white/60 border border-purple-900/5 rounded-[2.5rem] p-8">
            <div className="flex items-center gap-3 mb-8">
              <Clock className="text-indigo-400" size={20} />
              <h3 className="text-lg font-black text-slate-900 uppercase tracking-tight">Gelecek Yayınlar</h3>
            </div>

            <div className="space-y-4">
              {upcomingPrograms.map((item) => (
                <div key={item.id} className="flex items-center gap-4 p-3 rounded-2xl bg-white/[0.02] border border-purple-900/5 hover:bg-white/[0.05] transition-all group">
                  ...
                  <div className="flex-1 min-w-0">
                    <h5 className="text-[11px] font-black text-slate-900 uppercase tracking-tight truncate">{item.series_name}</h5>
              ...
              {upcomingPrograms.length === 0 && (
                <p className="text-[10px] text-slate-600 text-center italic font-bold uppercase tracking-widest py-4">Sırada yayın yok</p>
              )}
            </div>

            <button className="w-full mt-8 py-4 rounded-2xl bg-slate-900/5 border border-purple-900/5 text-[10px] font-black text-zinc-500 uppercase tracking-widest hover:bg-slate-900/10 hover:text-slate-900 transition-all">
              TÜM TAKVİMİ GÖR
            </button>
          </div>
        </div>
