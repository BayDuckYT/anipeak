  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="relative z-[1001] w-full max-w-[400px] bg-[#0c0a10] glass-strong rounded-[2rem] p-6 shadow-[0_0_50px_rgba(168,85,247,0.15)] border border-purple-900/10 overflow-y-auto max-h-full no-scrollbar"
      >
          {/* Glows */}
          ...
          <button onClick={onClose} className="absolute top-4 right-4 p-1.5 rounded-lg text-slate-600 hover:text-slate-900 hover:bg-slate-900/10 transition-all">
            <X size={18} />
          </button>
          ...
          {/* Tabs */}
          <div className="flex bg-slate-900/5 rounded-xl p-1 mb-5">
            {['login', 'register'].map((t) => (
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-lg text-sm font-semibold transition-all duration-200 ${
                  tab === t ? 'bg-gradient-to-r from-purple-600 to-blue-600 text-slate-900 shadow-neon-purple' : 'text-slate-600 hover:text-slate-900'
                }`}
              >
          ...
          {!success && tab !== 'confirm_email' && (
            <AnimatePresence mode="wait">
              ...
              >
                {tab === 'register' && (
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-slate-900/5 border border-purple-900/10 rounded-xl px-4 py-3 text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:border-purple-500 focus:shadow-[0_0_12px_rgba(168,85,247,0.4)] transition-all"
                    />
                  </div>
                )}
                <div>
                  <label className="block text-xs font-semibold text-slate-600 mb-1.5 uppercase tracking-wider">E-posta</label>
                  <input
                    name="email"
                    value={form.email}
                    onChange={handleChange}
                    ...
                    className="w-full bg-slate-900/5 border border-purple-900/10 rounded-xl px-4 py-3 text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:border-blue-500 focus:shadow-[0_0_12px_rgba(59,130,246,0.4)] transition-all"
                  />
                </div>
                {tab !== 'forgot' && (
                  <div>
                    ...
                    <div className="relative">
                      <input
                        ...
                        required
                        placeholder="••••••••"
                        className="w-full bg-slate-900/5 border border-purple-900/10 rounded-xl px-4 py-3 pr-11 text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:border-purple-500 focus:shadow-[0_0_12px_rgba(168,85,247,0.4)] transition-all"
                      />
                      <button type="button" onClick={() => setShowPass(!showPass)} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 hover:text-slate-700 transition-colors">
                ...
                {tab === 'register' && (
                  <div>
                    <label className="block text-xs font-semibold text-slate-600 mb-1.5 uppercase tracking-wider">Şifre Tekrarı</label>
                    <div className="relative">
                      <input
                        name="password2"
                        ...
                        required
                        placeholder="••••••••"
                        className="w-full bg-slate-900/5 border border-purple-900/10 rounded-xl px-4 py-3 pr-11 text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:border-blue-500 focus:shadow-[0_0_12px_rgba(59,130,246,0.4)] transition-all"
                      />
                      <button type="button" onClick={() => setShowPass(!showPass)} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 hover:text-slate-700 transition-colors">
                        {showPass ? <EyeOff size={16} /> : <Eye size={16} />}
                      </button>
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full py-3 rounded-xl text-sm font-bold bg-gradient-to-r from-purple-600 to-blue-600 text-slate-900 hover:from-purple-500 hover:to-blue-500 transition-all shadow-neon-purple disabled:opacity-60 flex items-center justify-center gap-2"
                >
                  {loading ? (
                ...
                {tab !== 'forgot' && (
                  <>
                    ...
                    <button
                      ...
                      disabled={loading}
                      className="w-full py-3 rounded-xl text-sm font-medium border border-purple-900/10 text-slate-700 hover:border-purple-900/20 hover:bg-slate-900/5 transition-all flex items-center justify-center gap-2"
                    >
