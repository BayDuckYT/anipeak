...
// ÔöÇÔöÇÔöÇ GER├çEK VER─░ BAZLI RUH T─░PLER─░ ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
const SOUL_TYPES = [
  { name: 'Shadow Wanderer', title: 'G├Âlge Gezgini', desc: 'Karanl─▒k ve gizemin derinliklerinde y├╝r├╝yen bir ruh.', color: 'purple', genres: ['Horror', 'Korku', 'Dark Fantasy', 'Karanl─▒k Fantezi', 'Gerilim', 'Thriller', 'Psikolojik'] },
  { name: 'Celestial Knight', title: 'G├Âk ┼Ş├Âvalyesi', desc: 'Adalet ve ─▒┼ş─▒─ş─▒n pe┼şinde, aksiyon dolu bir kader.', color: 'cyan', genres: ['Aksiyon', 'Action', 'Shounen', 'Macera', 'Adventure', 'D├Âv├╝┼ş Sanatlar─▒'] },
  { name: 'Nebula Dreamer', title: 'Nebula Hayalperesti', desc: 'Ger├ğekli─şin ├Âtesindeki d├╝nyalarda kaybolan bir zihin.', color: 'pink', genres: ['Fantezi', 'Fantasy', 'Isekai', 'Bilim Kurgu', 'Sci-Fi'] },
  { name: 'Silent Reaper', title: 'Sessiz Azrail', desc: 'Sessizli─şin i├ğindeki kaosu ve dram─▒ hisseden bir bilin├ğ.', color: 'indigo', genres: ['Drama', 'Romantik', 'Romance', 'Seinen', 'Slice of Life', 'G├╝nl├╝k Ya┼şam'] },
  { name: 'Void Hunter', title: 'Bo┼şluk Avc─▒s─▒', desc: 'Kaybolmu┼ş serilerin ve gizli kalm─▒┼ş hikayelerin ka┼şifi.', color: 'blue', genres: ['Gizem', 'Mystery', 'Dedektif', 'Su├ğ', 'Crime'] }
];
...
export default function OraclePage() {
  ...
  useSEO({
    title: 'Oracle',
    description: 'MahoraPeak Oracle - Yapay zeka destekli ki┼şisel manhwa ve webtoon ├Ânerileri.',
    url: 'https://mahorapeak.com.tr/oracle'
  });

  if (!sortedSeries || sortedSeries.length === 0) {
    return <Loader fullScreen={false} text="Sistem Ba┼şlat─▒l─▒yor..." />;
  }
  ...
  const { soulProfile, userStats } = useMemo(() => {
    ...
    let readingRhythm = 'Ke┼şfedici';
    if (xp > 10000) readingRhythm = 'Yo─şun';
    else if (xp > 5000) readingRhythm = 'D├╝zenli';
    else if (xp > 1000) readingRhythm = 'Aktif';
    else if (xp > 100) readingRhythm = 'Ba┼şlang─▒├ğ';
    ...
    const calculatedSoulProfile = {
      ...
      cosmicDistribution: cosmicDistribution.length > 0 ? cosmicDistribution : [{ label: 'Hen├╝z Veri Yok', value: 100 }]
    };
  ...
  const systemRank = useMemo(() => {
    if (!user) return { value: 'Misafir', subtext: 'Giri┼ş yaparak r├╝tbenizi g├Âr├╝n.' };
    const levelInfo = getLevelInfo(user.xp || 0, user.is_elite);
    return { value: levelInfo.rank, subtext: user.discord_id ? `Discord ile senkronize ┬À Lv.${levelInfo.level}` : `Lv.${levelInfo.level} ┬À Discord ba─şlant─▒s─▒ bekleniyor` };
  }, [user]);
  ...
  return (
    <div className="relative min-h-screen bg-[#070511] pb-20 text-white selection:bg-cyan-500/30 overflow-x-hidden">
      ...
      {/* ÔöÇÔöÇ NETFLIX TOP HERO (RUH E┼ŞLE┼ŞMEN) ÔöÇÔöÇ */}
      <div className="relative w-full h-[80vh] min-h-[600px] mb-12">
        ...
        <div className="relative z-20 w-full max-w-[1400px] mx-auto px-4 sm:px-12 h-full flex flex-col justify-end pb-24">
          <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="flex items-center gap-2 mb-4 px-3 py-1 bg-cyan-500/10 backdrop-blur-md rounded-full w-fit border border-cyan-500/20">
            ...
            <span className="text-xs font-bold text-cyan-400 tracking-widest uppercase">%99 RUH E┼ŞLE┼ŞMES─░</span>
          </motion.div>
          ...
          <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.2 }} className="flex items-center gap-4 text-slate-300 text-sm font-bold mb-6">
            <span className="px-2 py-1 bg-emerald-500/20 text-emerald-400 rounded uppercase">{soulProfile.name} Se├ğimi</span>
            <span>ÔÇó</span>
          ...
          <motion.p initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.3 }} className="text-slate-200 text-lg sm:text-xl font-medium max-w-2xl mb-8 drop-shadow-md line-clamp-3">
            Algoritmam─▒z─▒n derinliklerinde sana en uygun hikaye bu. {topMatch?.summary || 'Ruh profilin bu maceran─▒n her karesinde kendinden bir par├ğa bulacak.'}
          </motion.p>
      ...
      {/* ÔöÇÔöÇ NETFLIX ROWS ÔöÇÔöÇ */}
      <div className="relative z-30 -mt-20">
        <NetflixRow title="Sistem ├ûneriyor" subtitle={`${soulProfile.name} DNA's─▒na Uygun`} items={row1} />
        <NetflixRow title="Karanl─▒k ve Derin" subtitle="Belki G├Âzden Ka├ğ─▒rm─▒┼şs─▒nd─▒r" items={row2} />
      </div>

      {/* ÔöÇÔöÇ SOUL PROFILE DASHBOARD ÔöÇÔöÇ */}
      <div className="relative z-30 max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-12 mt-24 mb-12">
        <div className="flex items-center gap-3 mb-8">
          ...
          <h2 className="text-3xl font-black text-white uppercase tracking-tight">ANAL─░Z <span className="text-cyan-400">RAPORU</span></h2>
        </div>
              ...
              <div className="relative z-10 flex flex-col md:flex-row items-center gap-10 h-full">
                ...
                <div className="flex-1 text-center md:text-left">
                  <div className="flex items-center justify-center md:justify-start gap-2 mb-2">
                    ...
                    <span className="text-xs font-mono text-cyan-400 tracking-widest uppercase">RUH MERTEBES─░</span>
                  </div>
                  ...
                  <p className="text-gray-400 text-sm leading-relaxed mb-6 max-w-lg">
                    {soulProfile.desc} Sistemdeki e┼şle┼şme oran─▒n─▒z %{soulProfile.dnaScore} oran─▒nda benzersiz bulundu.
                  </p>
          ...
          <div className="lg:col-span-4 space-y-6">
            <div className="p-6 rounded-3xl bg-[#141414] border border-white/5">
              ...
              <div className="space-y-3">
                ...
                <MetricBox icon={Zap} label="Etkile┼şim" value={soulProfile.engagementRate} subtext={`${userStats?.totalChaptersRead || 0} b├Âl├╝m`} color="cyan" />
                <MetricBox icon={Shield} label="Sistem R├╝tbesi" value={systemRank.value} subtext={systemRank.subtext} color="pink" />
              </div>
            </div>
          </div>
        </div>
