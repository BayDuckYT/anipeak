...
export default function PopularityPage() {
  ...
  return (
    <main className="min-h-screen bg-[#F8F5FF] pt-24 pb-20 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <header className="text-center mb-16">
          ...
          <motion.h1 initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.1 }} className="text-4xl sm:text-5xl lg:text-6xl font-black text-slate-900 uppercase tracking-tight mb-4">
            KÜRESEL <span className="text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 to-orange-500">POPÜLERLİK</span>
          </motion.h1>
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="text-slate-600 max-w-2xl mx-auto font-medium">
        ...
        {top10.length > 0 && (
          <section className="mb-20">
            <h2 className="text-2xl font-black text-slate-900 text-center uppercase tracking-widest mb-10 flex items-center justify-center gap-3">
        ...
        {remaining.length > 0 && (
          <section>
             <h2 className="text-xl font-black text-slate-700 uppercase tracking-widest mb-6 flex items-center gap-2 border-b border-purple-900/10 pb-4">
              <Flame className="text-orange-500" size={20} /> Diğer Popüler Eserler
            </h2>
            
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {remaining.map((item, idx) => (
                <Link key={item.id} to={`/manhwa/${item.id}`} className="flex items-center gap-4 p-4 rounded-2xl bg-slate-900/5 border border-purple-900/5 hover:bg-slate-900/10 hover:border-yellow-500/30 transition-all group">
                  <div className="w-10 text-center text-xl font-black text-slate-600 group-hover:text-yellow-400 transition-colors">
                    {idx + 11}
                  </div>
                  <div className="w-16 h-24 rounded-lg overflow-hidden bg-white flex-shrink-0 shadow-lg">
                    <img 
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="text-sm font-bold text-slate-900 mb-2 truncate group-hover:text-yellow-400 transition-colors">{item.title}</h3>
                    <div className="flex flex-wrap items-center gap-3">
                      <div className="flex items-center gap-1.5 px-2 py-1 bg-black/40 rounded-md border border-purple-900/10">
                        <Star size={12} className="text-amber-400 fill-amber-400" />
                        <span className="text-amber-400 font-bold text-xs">{item.rating}</span>
                      </div>
                      <div className="flex items-center gap-1.5 px-2 py-1 bg-black/40 rounded-md border border-purple-900/10">
                        <Eye size={12} className="text-blue-400" />
                        <span className="text-slate-700 font-medium text-xs">{item.reads_num || 0}</span>
                      </div>
                    </div>
                  </div>
                </Link>
